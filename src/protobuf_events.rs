...
impl Tokenizer {
    pub fn new(_reader :&mut dyn std::io::Read) -> Tokenizer {
        Tokenizer{}
    }
}
...
pub fn parse_file_descriptor_proto_set(r :&mut dyn Read) -> HashMap<String, HashMap<u32, SchemaField>> {
...
pub fn read_varu64(r: &mut dyn Read) -> Result<u64, ReadStatus> {// (uint64, error) {
	let mut x: u64 = 0;
	let mut s: u32 = 0;
	let i: u32 = 0;
	let mut buf = [0; 1];
...
pub fn read_fixedu32(r : &mut dyn Read) -> u32 {
    let mut buf = [0; 4];
...
pub fn read_fixedu64(r : &mut dyn Read) -> u64 {
...
pub fn parse_any_message(r :&mut dyn Read, type_map :&HashMap<u32, SchemaField>) {
...
pub fn decode_sfixed64(n :u64) -> i64 { return n as i64; }

pub fn decode_fixed64(n :u64) -> u64 { return n as u64; }

pub fn decode_double(n :u64) -> f64 { return f64::from_bits(n); }

pub fn decode_sfixed32(n :u32) -> i32 { return n as i32;}

pub fn decode_fixed32(n :u32) -> u32 { return n;}

pub fn decode_float(n :u32) -> f32 { return f32::from_bits(n); }
...
mod test {
    ...
    fn zigzag32_decoding() {
        assert_eq!(-1, super::decode_zigzag32(1));
        assert_eq!(1, super::decode_zigzag32(2));
        assert_eq!(-2, super::decode_zigzag32(3));
        assert_eq!(2147483647, super::decode_zigzag32(4294967294));
        assert_eq!(-2147483648, super::decode_zigzag32(4294967295));
    }

    #[test]
    fn zigzag64_decoding() {
        assert_eq!(-1, super::decode_zigzag64(1));
        assert_eq!(1, super::decode_zigzag64(2));
        assert_eq!(-2, super::decode_zigzag64(3));
        assert_eq!(2147483648, super::decode_zigzag64(4294967296));
        assert_eq!(-2147483649, super::decode_zigzag64(4294967297));
    }

    #[test]
    fn float_decoding() {
        let a = [205, 204, 204, 63];
        let n = super::read_fixedu32(&mut &a[..]);
        let f = super::decode_float(n);
        assert_eq!(1.6, f);
    }
    ...
    fn double_decoding() {
        ...
        let n = super::read_fixedu64(&mut &a[..]);
        let f = super::decode_double(n);
        assert_eq!(2.4, f);
    }
    ...
    fn test_read_varu64_to_few_byte(){
        ...
        match super::read_varu64(&mut one_short) {
            Ok(_) => assert_eq!(1, 2),
            Err(v) => match v { // we shouldn't end up here
                super::ReadStatus::Empty => (),
                _ => assert!(false)
            }
        }
    }
    ...
    fn test_tokenizer_new() {
        super::Tokenizer::new(&mut "".as_bytes());
    }
}
